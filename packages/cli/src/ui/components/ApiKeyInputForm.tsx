...
export function ApiKeyInputForm({
  serviceName,
  onSubmit,
  onCancel,
}: ApiKeyInputFormProps) {
  const [apiKey, setApiKey] = useState('');
  const [focusedField, setFocusedField] = useState<'input' | 'submit' | 'cancel'>('input');
  const [submitError, setSubmitError] = useState<string | null>(null);

  const handleSubmit = () => {
    // 如果用户没有输入apiKey，直接按回车，则设置为'aihc-no-auth'
    const finalApiKey = apiKey.trim() || 'aihc-no-auth';
    setSubmitError(null);
    onSubmit(finalApiKey);
  };
  ...
  return (
    ...
    >
      ...
      <Box marginTop={1}>
        <Text>The selected service &quot;{serviceName}&quot; requires an API key.</Text>
      </Box>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Note: Press Enter to use &apos;aihc-no-auth&apos; if you don&apos;t have an API key.
        </Text>
      </Box>
